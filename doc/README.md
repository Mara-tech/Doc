...
```bash
doc$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
doc$ sam local start-api
doc$ curl http://localhost:3000/
```
...
```bash
doc$ sam logs -n HelloWorldFunction --stack-name doc --tail
```
...
```bash
doc$ pip install pytest pytest-mock --user
doc$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name doc
```
